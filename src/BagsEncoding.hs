module BagsEncoding where

import qualified Data.Map as M
import Data.Map (Map)
import Language.Haskell.Liquid.ProofCombinators
-- https://github.com/ucsd-progsys/liquidhaskell/issues/1328
...
-- {-@ thm_bags_emp :: x:k -> xs:Bag k ->  { empty /= put x xs }  @-}
-- thm_bags_emp :: (Ord k) => k -> Bag k -> Proof
-- thm_bags_emp x xs = get x xs *** QED

-- {-@ appendBag :: as:[k] -> bs:[k] -> {fromList (as ++ bs) == union (fromList as) (fromList bs) } @-}
-- appendBag :: Ord k => [k] -> [k] -> Proof
-- appendBag [] _ = trivial
-- appendBag (a:as) bs = appendBag as bs
